...
class TestUpdateCustomerProfile:
    ...
    @pytest.mark.parametrize('invalid_name, alert_msg', [
        ...
        ("John", BankAlert.NAME_MUST_CONTAIN_TWO_WORDS),
        ("John123 Smith", BankAlert.NAME_MUST_CONTAIN_TWO_WORDS),
        ("John@ Smith", BankAlert.NAME_MUST_CONTAIN_TWO_WORDS),
        ("John Michael Smith", BankAlert.NAME_MUST_CONTAIN_TWO_WORDS),
    ])
    def test_update_customer_profile_invalid_name(self, page: Page, api_manager: ApiManager,
                                                  user_request: CreateUserRequest,
                                                  invalid_name: str, alert_msg: str):
        UserDashboard(page).open()\
            .click_username_in_header()\
